...
namespace Qtud.DBManage.DAL
{
    public class PrintInfoDal
    {
		...
		/// 增加一条数据
		/// </summary>
		public void Add(Qtud.DBManage.Model.PrintInfoModel model)
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("insert into tbl_print_info(");
            strSql.Append("uuid,useruuid,ReportUUId ,pagecnt ,printdate,printcnt )");
			strSql.Append(" values (");
			strSql.Append(@"'"+ model.uuid +@"', ");
			strSql.Append(@"'"+ model.useruuid +@"', ");
			strSql.Append(@"'"+ model.ReportUUId +@"', ");
			strSql.Append( model.pagecnt +@", ");
			strSql.Append(@"'"+ model.printDate +@"', ");
            strSql.Append(model.printcnt + @"  );");
